...
- Gemini API로 여러 시트의 템플릿 셀 위치와 업로드 데이터 컬럼을 함께 자동 매핑
- 여러 시트가 있는 템플릿의 전체 시트 구조를 프롬프트에 포함
- 사이드바에서 템플릿 분석 행/열 범위를 조정
- CSV/XLSX 일일 데이터 업로드
- 매핑 JSON 기반으로 업로드 데이터 전체 행을 교체
...
5. 필요하면 매핑 JSON을 직접 수정합니다. 여러 시트 템플릿이면 `sheets` 배열에 시트별 매핑을 넣습니다.
6. 보고일자와 비고를 입력한 뒤 실행합니다.

매핑 JSON의 `sheets`는 시트별 교체 규칙입니다. 각 시트 매핑의 `data_table.columns`는 템플릿 엑셀에서 값을 넣을 컬럼 위치이고, `source_columns`는 업로드 데이터에서 값을 읽을 컬럼명입니다.
